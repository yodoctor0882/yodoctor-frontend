...
const StatCard = ({ label, value, icon, color }) => (
  <div className="bg-white rounded-2xl border border-slate-200/80 px-6 py-5 flex items-center gap-5 shadow-sm">
    ...
    <div>
      <p className="text-3xl font-bold text-slate-800 font-[family-name:var(--font-dm)] leading-none">{value}</p>
      <p className="text-sm text-slate-400 font-[family-name:var(--font-dm)] mt-1">{label}</p>
    </div>
  </div>
);

/* ── Doctor Card ── */
const DoctorCard = ({ d, onView, onAction, activeMenu, setActiveMenu, imageError, setImageError }) => {
  const st = STATUS[d.status] || STATUS.PENDING;
  const initials = d.doctorName?.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase();
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm hover:shadow-md transition-shadow duration-200 relative">
      ...
      {/* Status badge */}
      <span className={`absolute top-5 right-5 text-xs font-bold px-3 py-1.5 rounded-full border font-[family-name:var(--font-dm)] ${st.cls}`}>
        {st.label}
      </span>
      ...
      <div className="flex gap-4 pr-24">
        ...
        <div className="w-16 h-16 rounded-2xl overflow-hidden bg-slate-100 flex-shrink-0 border border-slate-200">
          {d.profile_image && !imageError[d.id] ? (
            <img
              ...
              onError={() => setImageError(prev => ({ ...prev, [d.id]: true }))}
            />
          ) : (
        ...
        <div className="flex-1 min-w-0">
          <p className="text-base font-bold text-slate-800 truncate font-[family-name:var(--font-dm)]">{d.doctorName}</p>
          <p className="text-sm font-semibold text-blue-600 mt-0.5 font-[family-name:var(--font-dm)]">{d.specialization}</p>
        ...
        {d.status === "PENDING" && (
          <div className="relative">
            ...
            {activeMenu === d.id && (
              ...
              >
                <button
                  onClick={() => { onAction("APPROVE", d); setActiveMenu(null); }}
                ...
                <button
                  onClick={() => { onAction("REJECT", d); setActiveMenu(null); }}
...
const DocsNotVerifiedModal = ({ doctor, unverifiedCount, onClose, onGoVerify, navigate }) => (
  <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
    <div className="bg-white rounded-2xl border border-slate-200 p-8 w-full max-w-sm shadow-2xl">
      ...
      <p className="text-sm text-slate-400 text-center font-[family-name:var(--font-dm)] mb-2">
        <span className="font-semibold text-slate-600">{doctor?.doctorName}</span> {" "}
        <span className="font-bold text-amber-600">{unverifiedCount} document(s)</span> are pending.
      </p>
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={() => { onClose(); navigate(`/admin/doctorsdetails/${doctor?.id}`); }}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-2xl border border-slate-200 p-8 w-full max-w-sm shadow-2xl animate-[var(--animate-scale-in)]">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-5 ${isApprove ? "bg-emerald-50" : "bg-red-50"}`}>
          {isApprove ? (
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth={2.5} strokeLinecap="round"><path d="M5 13l4 4L19 7"/></svg>
          ) : (
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth={2.5} strokeLinecap="round"><path d="M6 18L18 6M6 6l12 12"/></svg>
          )}
        </div>
        <h3 className="text-lg font-bold text-slate-800 text-center font-[family-name:var(--font-dm)] mb-2">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} disabled={processing}
            ...
            {processing ? "Processing..." : isApprove ? "Yes, Approve" : "Yes, Reject"}
          </button>
        </div>
      </div>
    </div>
...
const AdminDoctor = () => {
  ...
  const [confirmModal, setConfirmModal] = useState({ open: false, type: "", doctor: null });
  const [docsWarningModal, setDocsWarningModal] = useState({ open: false, doctor: null, unverifiedCount: 0, pendingAction: null });
  ...
  const handleConfirmAction = async () => {
    ...
    if (!doctor?.id) { notify.error("Invalid doctor"); return; }
    try {
      setProcessing(true);
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-slate-400 text-sm font-[family-name:var(--font-dm)]">Loading doctors...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 sm:px-6 lg:px-10 py-8">
      ...
      {checkingDocs && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-40">
          <div className="bg-white rounded-2xl border border-slate-200 px-8 py-6 shadow-xl flex items-center gap-4">
            ...
            <p className="text-sm font-semibold text-slate-600 font-[family-name:var(--font-dm)]">Checking documents...</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <StatCard label="Total Doctors" value={stats.pending + stats.approved + stats.rejected}
          color="bg-blue-50"
          icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth={2} strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>}
        />
        <StatCard label="Pending Review" value={stats.pending}
          color="bg-amber-50"
          icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth={2} strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>}
        />
        <StatCard label="Approved" value={stats.approved}
          color="bg-emerald-50"
          icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth={2.5} strokeLinecap="round"><path d="M5 13l4 4L19 7"/></svg>}
        />
        <StatCard label="Rejected" value={stats.rejected}
          color="bg-red-50"
          icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth={2.5} strokeLinecap="round"><path d="M6 18L18 6M6 6l12 12"/></svg>}
        />
      </div>
      ...
      <div className="flex justify-between flex-col sm:flex-row gap-3 mb-7">
        <div className="relative flex-1 max-w-sm">
          <svg className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round">
        ...
        <div className="flex gap-2 flex-wrap">
          {["ALL","PENDING","APPROVED","REJECTED"].map(s => (
            <button key={s} onClick={() => { setStatusFilter(s); setVisibleCount(PAGE_SIZE); }}
              className={[
      ...
      {visibleDoctors.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 gap-3">
          <div className="w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center">
            <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth={1.5} strokeLinecap="round">
              <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/>
              <path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/>
            </svg>
          </div>
          <p className="text-base font-semibold text-slate-500 font-[family-name:var(--font-dm)]">No doctors found</p>
          <p className="text-sm text-slate-400 font-[family-name:var(--font-dm)]">Try changing the search or filter</p>
        </div>
      )}
      ...
      {docsWarningModal.open && (
        <DocsNotVerifiedModal
          ...
          onClose={() => setDocsWarningModal({ open: false, doctor: null, unverifiedCount: 0, pendingAction: null })}
          navigate={navigate}
        />
      )}
      ...
      {confirmModal.open && (
        <ConfirmModal
          ...
          onCancel={() => setConfirmModal({ open: false, type: "", doctor: null })}
